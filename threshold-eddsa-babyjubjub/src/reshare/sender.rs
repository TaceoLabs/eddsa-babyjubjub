...
impl<C: CurveGroup> ReShareProtocolSender<C> {
    ...
    /// run. It is the only run-specific input to the proof-of-possession context, so reusing it with
    /// the same parameters makes this broadcast replayable into a later run, where the sender's
    /// fresh evaluations fail against the stale commitments and the honest sender is reported as
    /// malicious. Use fresh random bytes per run. The context is not carried in the protocol messages: a
    /// participant running with a different context fails proof verification at the receivers and
    /// is reported as malicious.
    ///
    /// # Errors
    /// Returns an error if `party_index` is larger than the number of parties allowed by the old
    ...
    pub fn new<R: Rng + CryptoRng>(
        ...
        let proof_context =
            schnorr::proof_context(Self::CONTEXT_DOMAIN, context, &[old_params, new_params]);
    ...
    /// [`Parameters`](crate::keygen::Parameters).
    pub fn get_party_communication(&self, for_party: NonZeroU16) -> Result<PartyMessage<C>> {
        ...
        let secret_share =
            lagrange::evaluate_poly(&self.coefficients, C::ScalarField::from(for_party.get()));

        Ok(PartyMessage { secret_share })
    }
}
