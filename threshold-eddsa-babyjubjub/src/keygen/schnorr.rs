...
impl<C: CurveGroup> SchnorrZkProof<C> {
    ...
    fn challenge_hash(
        ...
        random_oracle.update(&party_idx.get().to_be_bytes());
...
/// Builds the proof-of-possession context from the protocol domain, the opaque caller-supplied
/// session context, and the run's [`Parameters`].
///
/// The variable-length session context is length-prefixed to keep the encoding injective.
pub(crate) fn proof_context(
    domain: &[u8],
    session_context: &[u8],
    parameters: &[Parameters],
) -> Vec<u8> {
    let len = domain.len() + 8 + session_context.len() + parameters.len() * 4;
    let mut context = Vec::with_capacity(len);
    context.extend_from_slice(domain);
    context.extend_from_slice(
        &u64::try_from(session_context.len())
            .expect("context length fits into u64")
            .to_be_bytes(),
    );
    context.extend_from_slice(session_context);
    for parameters in parameters {
        context.extend_from_slice(&parameters.number_of_parties.get().to_be_bytes());
        context.extend_from_slice(&parameters.threshold.get().to_be_bytes());
    }
...
mod tests {
    ...
    fn basic_schnorr_nizk() {
        ...
        let idx = NonZeroU16::new(1).expect("non-zero");
