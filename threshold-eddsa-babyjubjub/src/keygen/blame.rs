...
pub struct BlameVerdict {
    #[serde(deserialize_with = "crate::internal::serde::deserialize_protocol_btree_set")]
    pub(crate) blamed_parties: BTreeSet<NonZeroU16>,
}

impl BlameVerdict {
    /// Returns the parties accused by this verdict. An empty set represents `Ok`.
    #[must_use]
    pub fn blamed_parties(&self) -> &BTreeSet<NonZeroU16> {
        &self.blamed_parties
    }
...
pub struct BlameRevelation<C: CurveGroup> {
    #[serde(
        serialize_with = "crate::internal::serde::serialize_protocol_vec",
        deserialize_with = "crate::internal::serde::deserialize_protocol_vec"
    )]
...
pub struct BlameResult<C: CurveGroup> {
    ...
    pub disqualified_parties: Vec<NonZeroU16>,
    ...
    pub excluded_verdict_parties: Vec<NonZeroU16>,
}

/// State for the optional public DKG complaint and blame workflow.
pub struct BlameRound<C: CurveGroup> {
    context: Vec<u8>,
    commitments: HashMap<NonZeroU16, Vec<C::Affine>>,
    secret_shares: SecretScalars<C::ScalarField>,
    my_idx: NonZeroU16,
    params: Parameters,
    received_party_messages: SecretScalarMap<C::ScalarField>,
    verdicts: BTreeMap<NonZeroU16, BlameVerdict>,
    resolved_dealers: BTreeSet<NonZeroU16>,
    disqualified_dealers: BTreeSet<NonZeroU16>,
}
...
impl<C: CurveGroup> BlameRound<C> {
    ...
    ///
    /// # Errors
    /// Returns an error for an invalid sender, duplicate verdict, or invalid blamed party
    /// identifier.
    pub fn add_verdict(&mut self, from: NonZeroU16, verdict: BlameVerdict) -> Result<()> {
    ...
    #[must_use]
    pub fn missing_verdicts(&self) -> Vec<NonZeroU16> {
        self.commitments
            .keys()
    ...
    pub fn disqualify_missing_verdict(&mut self, party: NonZeroU16) -> Result<()> {
    ...
    pub fn accusations(&self) -> Result<BTreeMap<NonZeroU16, BTreeSet<NonZeroU16>>> {
        if !self.verdicts_complete() {
            eyre::bail!("cannot determine accusations before all verdicts are received");
        }
        let mut accusations = BTreeMap::<NonZeroU16, BTreeSet<NonZeroU16>>::new();
    ...
    pub fn revelation(&mut self) -> Result<Option<BlameRevelation<C>>> {
        ...
        let threshold = usize::from(self.params.threshold.get());
        ...
        let shares = accusers
            ...
            .map(|receiver| RevealedShare {
                receiver: *receiver,
                share: self.secret_shares[usize::from(receiver.get()) - 1],
            })
            .collect();
        Ok(Some(BlameRevelation { shares }))
    }
    ...
    pub fn add_revelation(
        &mut self,
        from: NonZeroU16,
        revelation: &BlameRevelation<C>,
    ) -> Result<()> {
        ...
        let structurally_valid = !duplicate_receiver && &revealed_receivers == expected_receivers;
        let equations_valid = structurally_valid
            && revelation.shares.iter().all(|revealed| {
                lagrange::verify_polynomial_evaluation::<C>(
    ...
    pub fn disqualify_missing_dealer(&mut self, dealer: NonZeroU16) -> Result<()> {
    ...
    /// # Errors
    /// Returns an error until all verdict broadcasts have been collected.
    pub fn missing_revelations(&self) -> Result<Vec<NonZeroU16>> {
        Ok(self
            .accusations()?
            .keys()
    ...
    pub fn finalize(self) -> Result<BlameResult<C>> {
        ...
        if qualified_dealers.len() < usize::from(self.params.threshold.get()) {
            ...
            qualified_dealers
                ...
                .fold(C::ScalarField::zero(), |acc, dealer| {
                    if *dealer == self.my_idx {
                        acc + self.secret_shares[usize::from(self.my_idx.get()) - 1]
                    } else {
        ...
        for party in shareholders {
            let share = qualified_dealers
                ...
                .fold(C::zero(), |acc, dealer| {
                    acc + lagrange::evaluate_polynomial_in_exponent::<C>(
    ...
    fn validate_other_party(&self, from: NonZeroU16) -> Result<()> {
        if from > self.params.number_of_parties {
            eyre::bail!("party index {from} invalid for parameters",);
        }
        if from == self.my_idx {
